...
```c
void generate_mod_h_external(Domain* mod, const char* dir)
```
...
```c
void generate_mod_test(Domain* mod, const char* dir)
```
...
```c
int test_is_string_type(const char* type)
```
...
```c
const char* test_fmt(const char* type)
```
...
```c
void test_emit_expected(FILE* f, const char* type, const char* expected)
```
