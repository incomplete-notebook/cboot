 ...
 * .cboot script parser (新规范 v0.5.0)
 *
 * 支持 .cboot 文件引用语法:
...
static int parser_dispatch_modify(char **tokens, int count) {
    ...
    /* 修改字段: test <cov> <pass> (函数测试目标, 可选) */
    if (utils_str_eq(cmd, "test")) {
        if (parser_require_args(tokens, count, 3, "test <覆盖率> <通过率>") < 0) return -1;
        int cov = atoi(tokens[1]);
        int pass = atoi(tokens[2]);
        return commands_cmd_test(cov, pass);
    }
