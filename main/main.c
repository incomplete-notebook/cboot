/* main.c - CBoot generated (compiler: exe) */
/* Module: main */

/*
 * CBoot - C Project Bootstrapping Tool v0.5.0
 * Main entry point (新规范 v0.5.0)
 */

#include "cboot.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <termios.h>

/* ------------------------------------------------------------------ */
/* Global state                                                       */
/* ------------------------------------------------------------------ */

Project *g_proj   = NULL;
RunMode  g_mode   = MODE_INTERACTIVE;
int      g_force  = 0;
int      g_running = 1;
int      g_skip_gen = 0;  /* 加载 .cboot 时跳过 gen 命令 (analyze 使用) */

/* ------------------------------------------------------------------ */
/* Forward declarations                                               */
/* ------------------------------------------------------------------ */

static void main_print_usage(const char *prog);
static void main_repl_loop(void);
static int  main_dispatch_command(char **tokens, int count);
static int  main_detect_fine_tune_mode(void);

/* 解析命令行参数；返回 0 表示继续，非 0 表示已处理（应直接退出）。 */
/* 处理无值标志参数：返回 1 表示已处理，0 表示未匹配 */
static int main_parse_flag(const char *arg, int *adjust_mode, int *analyze_mode,
                           int *update_mode) {
    if (utils_str_eq(arg, "-f") || utils_str_eq(arg, "--force")) { g_force = 1; return 1; }
    if (utils_str_eq(arg, "-update")) { *update_mode = 1; return 1; }
    if (utils_str_eq(arg, "adjust")) { *adjust_mode = 1; return 1; }
    if (utils_str_eq(arg, "analyze")) { *analyze_mode = 1; return 1; }
    return 0;
}

/* 处理带帮助/版本输出的标志：返回 1 表示已处理并应退出 */
static int main_parse_info_flag(const char *arg, const char *prog) {
    if (utils_str_eq(arg, "-h") || utils_str_eq(arg, "--help")) {
        main_print_usage(prog);
        return 1;
    }
    if (utils_str_eq(arg, "-v") || utils_str_eq(arg, "--version")) {
        printf("CBoot V%s\n", CBOOT_VERSION);
        return 1;
    }
    return 0;
}

/* 处理位置参数（非 - 开头）：.cboot 脚本或项目名 */
static void main_parse_positional(const char *arg,
                                  const char **proj_name, const char **script_file) {
    int len = (int)strlen(arg);
    if (len >= 6 && strcmp(arg + len - 6, ".cboot") == 0) {
        *script_file = arg;
    } else {
        *proj_name = arg;
    }
}

static int main_parse_args(int argc, char **argv,
                           const char **proj_name,
                           const char **script_file,
                           int *adjust_mode, int *analyze_mode, int *update_mode) {
    for (int i = 1; i < argc; i++) {
        const char *arg = argv[i];

        if (main_parse_flag(arg, adjust_mode, analyze_mode, update_mode)) continue;
        if (main_parse_info_flag(arg, argv[0])) return 1;

        if (arg[0] == '-') {
            fprintf(stderr, "cboot: 未知选项 '%s'\n", arg);
            main_print_usage(argv[0]);
            return -1;
        }
        main_parse_positional(arg, proj_name, script_file);
    }
    return 0;
}

/* update 模式：自动识别 .cboot，加载项目，执行 update 同步后自动 analyze */
static int main_run_update(void) {
    if (!utils_file_exists(".cboot")) {
        fprintf(stderr, "cboot: 当前目录没有 .cboot 文件\n");
        return 1;
    }
    g_mode = MODE_BATCH;
    g_proj = domain_project_new("cboot_project");
    g_skip_gen = 1;  /* 加载阶段跳过 gen，仅同步已有源码 */
    if (parser_parse_cboot_script(".cboot") != 0) {
        fprintf(stderr, "cboot: 无法加载 .cboot 文件\n");
        g_skip_gen = 0;
        domain_project_free(g_proj); g_proj = NULL;
        return 1;
    }
    g_skip_gen = 0;
    printf("CBoot V%s - update 模式 (自动识别 .cboot)\n", CBOOT_VERSION);
    int rc = commands_cmd_update();
    /* update 后自动执行 analyze */
    printf("\n===== 代码分析 =====\n");
    commands_cmd_analyze();
    domain_project_free(g_proj); g_proj = NULL;
    return rc;
}

/* adjust 模式：加载已有项目，update 同步后进入交互式 REPL */
static int main_run_adjust(void) {
    if (!utils_file_exists(".cboot")) {
        fprintf(stderr, "cboot: 当前目录没有 .cboot 文件\n");
        return 1;
    }
    g_mode = MODE_INTERACTIVE;
    g_proj = domain_project_new("cboot_project");
    if (parser_parse_cboot_script(".cboot") != 0) {
        fprintf(stderr, "cboot: 无法加载 .cboot 文件\n");
        domain_project_free(g_proj); g_proj = NULL;
        return 1;
    }
    printf("CBoot V%s - 微调模式 (adjust)\n", CBOOT_VERSION);
    printf("项目 '%s' 已加载。\n\n", g_proj->root->name);
    commands_cmd_adjust();
    main_repl_loop();
    domain_project_free(g_proj); g_proj = NULL;
    return 0;
}

/* analyze 模式：加载项目（跳过 gen），运行代码分析 */
static int main_run_analyze(void) {
    if (!utils_file_exists(".cboot")) {
        fprintf(stderr, "cboot: 当前目录没有 .cboot 文件\n");
        return 1;
    }
    g_mode = MODE_BATCH;
    g_proj = domain_project_new("cboot_project");
    g_skip_gen = 1;
    parser_parse_cboot_script(".cboot");
    g_skip_gen = 0;
    commands_cmd_analyze();
    domain_project_free(g_proj); g_proj = NULL;
    return 0;
}

/* 批处理模式：执行指定的 .cboot 脚本 */
static int main_run_batch_script(const char *script_file) {
    if (!utils_file_exists(script_file)) {
        fprintf(stderr, "cboot: 未找到脚本文件 '%s'\n", script_file);
        return 1;
    }
    g_mode = MODE_BATCH;
    g_proj = domain_project_new("cboot_project");
    printf("CBoot V%s - 批处理模式 (%s)\n", CBOOT_VERSION, script_file);
    if (parser_parse_cboot_script(script_file) != 0) {
        fprintf(stderr, "cboot: 脚本执行失败\n");
        domain_project_free(g_proj); g_proj = NULL;
        return 1;
    }
    domain_project_free(g_proj); g_proj = NULL;
    return 0;
}

/* 默认批处理模式：执行当前目录的 .cboot */
static int main_run_default_cboot(void) {
    if (!utils_file_exists(".cboot")) return -1;
    g_mode = MODE_BATCH;
    g_proj = domain_project_new("cboot_project");
    printf("CBoot V%s - 批处理模式 (.cboot)\n", CBOOT_VERSION);
    if (parser_parse_cboot_script(".cboot") != 0) {
        fprintf(stderr, "cboot: 脚本执行失败\n");
        domain_project_free(g_proj); g_proj = NULL;
        return 1;
    }
    domain_project_free(g_proj); g_proj = NULL;
    return 0;
}

/* 交互模式：创建项目目录并进入 REPL */
static int main_run_interactive(const char *proj_name, const char *prog) {
    g_mode = MODE_INTERACTIVE;
    if (!utils_is_valid_identifier(proj_name)) {
        fprintf(stderr, "cboot: 无效的项目名 '%s'（仅允许字母、数字、下划线）\n", proj_name);
        return 1;
    }
    if (g_force && utils_file_exists(proj_name)) {
        char cmd[MAX_PATH_LEN];
        snprintf(cmd, sizeof(cmd), "rm -rf %s", proj_name);
        system(cmd);
    }
    if (utils_file_exists(proj_name) && !g_force) {
        fprintf(stderr, "cboot: 目录 '%s' 已存在。使用 -f 强制重建。\n", proj_name);
        return 1;
    }
    utils_ensure_dir(proj_name);
    if (chdir(proj_name) != 0) {
        perror("cboot: chdir");
        return 1;
    }
    g_proj = domain_project_new(proj_name);
    printf("CBoot V%s - 交互模式\n", CBOOT_VERSION);
    printf("项目 '%s' 已创建。输入命令开始构建，输入 help 查看帮助。\n\n", proj_name);
    main_repl_loop();
    domain_project_free(g_proj); g_proj = NULL;
    return 0;
}

int main(int argc, char **argv) {
    const char *proj_name = NULL;
    const char *script_file = NULL;
    int adjust_mode = 0, analyze_mode = 0, update_mode = 0;

    int rc = main_parse_args(argc, argv, &proj_name,
                             &script_file, &adjust_mode, &analyze_mode, &update_mode);
    if (rc != 0) return rc < 0 ? 1 : 0;

    if (update_mode)            return main_run_update();
    if (adjust_mode)            return main_run_adjust();
    if (analyze_mode)           return main_run_analyze();
    if (script_file)            return main_run_batch_script(script_file);
    if (!proj_name) {
        rc = main_run_default_cboot();
        if (rc != -1) return rc;
        fprintf(stderr, "cboot: 未指定项目名或 .cboot 脚本\n");
        main_print_usage(argv[0]);
        return 1;
    }
    return main_run_interactive(proj_name, argv[0]);
}

/* ------------------------------------------------------------------ */
/* main_detect_fine_tune_mode - check if .cboot project has been generated  */
/* ------------------------------------------------------------------ */

static int main_detect_fine_tune_mode(void) {
    /* 检测是否已生成项目：检查 CMakeLists.txt 和 main.c */
    if (utils_file_exists("CMakeLists.txt")) {
        if (utils_file_exists("main.c")) {
            return 1;
        }
    }
    return 0;
}

/* ------------------------------------------------------------------ */
/* Usage                                                              */
/* ------------------------------------------------------------------ */

static void main_print_usage(const char *prog) {
    printf("CBoot V%s - C 项目引导工具\n\n", CBOOT_VERSION);
    printf("用法:\n");
    printf("  %s <项目名>            交互模式，创建项目并进入 REPL\n", prog);
    printf("  %s <项目名> -f         交互模式，强制重建项目\n", prog);
    printf("  %s <file.cboot>        执行指定的 .cboot 脚本文件\n", prog);
    printf("  %s -update             更新模式：自动识别 .cboot，同步源码后执行代码分析\n", prog);
    printf("  %s adjust              微调模式：update同步后进入交互式REPL\n", prog);
    printf("  %s analyze             分析模式：统计代码行数/圈复杂度/重复率\n", prog);
    printf("  %s                     批处理模式，读取当前目录 .cboot\n", prog);
    printf("  %s -v                  显示版本号\n", prog);
    printf("  %s -h                  显示帮助\n", prog);
    printf("\n交互模式命令:\n");
    printf("  建立域:\n");
    printf("    mod <名称>             创建模块\n");
    printf("    struct <名称>          创建结构体\n");
    printf("    type <名称>            创建类型\n");
    printf("    def <名称>             创建宏\n");
    printf("    void <名称> <类型>     创建函数\n");
    printf("    var <名称> <类型>      创建变量\n");
    printf("    mem <名称> <类型>      创建成员/参数\n");
    printf("    enum <defs> <start>    枚举式创建宏\n");
    printf("  修改字段:\n");
    printf("    cmt \"文本\"             设置注释\n");
    printf("    value <值>             设置值\n");
    printf("    mode <模式>            设置模式 (internal/external/api/normal/...)\n");
    printf("    cmode <模式>           设置编译器模式 (exe/sl/dl/normal)\n");
    printf("  控制:\n");
    printf("    cd <路径>              导航域树\n");
    printf("    rm <名称> [-f]         删除子域\n");
    printf("    mv <src> <target>      移动域\n");
    printf("    find <类型> <字符>     搜索域\n");
    printf("    ls [名称]              查看域内容\n");
    printf("  其他:\n");
    printf("    gen                    生成代码\n");
    printf("    update                 扫描源码同步 .cboot（更新 code 和 API 定义）\n");
    printf("    analyze                统计代码行数/圈复杂度/代码重复率\n");
    printf("    adjust                 update 同步后进入交互式 REPL\n");
    printf("    im <.cboot 文件>       导入脚本\n");
    printf("    res <文件>             添加资源\n");
    printf("    exit                   退出\n");
}

/* ------------------------------------------------------------------ */
/* Tab completion helpers                                               */
/* ------------------------------------------------------------------ */

/* All command names for tab completion */
static const char *g_cmd_names[] = {
    "mod", "struct", "type", "def", "void", "var", "mem", "enum",
    "cmt", "value", "call", "mode", "cmode", "code",
    "cd", "rm", "mv", "find", "ls",
    "gen", "update", "analyze", "adjust", "im", "in", "res", "exit", "help", "?",
    NULL
};

/* Domain type names for find command */
static const char *g_domain_types[] = {
    "mod", "void", "struct", "type", "def", "var", "mem",
    NULL
};

/* Mode values for mode command */
static const char *g_mode_values[] = {
    "src", "static", "dynamic", "external", "api", "normal",
    "rename", "struct", "api rename", "api struct",
    NULL
};

/* Compiler mode values for cmode command */
static const char *g_cmode_values[] = {
    "exe", "sl", "dl", "normal",
    NULL
};

/*
 * main_try_complete - given a prefix, find all matching strings.
 * Returns number of matches; fills matches[] with pointers.
 */
static int main_try_complete(const char *prefix, const char **candidates, int max_matches,
                        const char **matches)
{
    int count = 0;
    int plen = (int)strlen(prefix);
    for (int i = 0; candidates[i] && count < max_matches; i++) {
        if (strncmp(candidates[i], prefix, plen) == 0) {
            matches[count++] = candidates[i];
        }
    }
    return count;
}

/*
 * main_common_prefix_len - find the length of the longest common prefix
 * among the given strings.
 */
static int main_common_prefix_len(const char **strs, int count)
{
    if (count <= 0) return 0;
    int len = (int)strlen(strs[0]);
    for (int i = 1; i < count; i++) {
        int j = 0;
        while (j < len && strs[0][j] == strs[i][j]) j++;
        len = j;
    }
    return len;
}

/* helpers for main_do_tab_complete: gather candidate matches from current scope
 * where each candidate name starts with prefix; returns match count. */
static int tab_collect_child_domains(const char *prefix, const char **matches, int max_matches) {
    int plen = (int)strlen(prefix);
    int match_count = 0;
    Domain *cur = g_proj->current;
    for (int i = 0; i < cur->child_count && match_count < max_matches; i++) {
        if (strncmp(cur->children[i]->name, prefix, plen) == 0)
            matches[match_count++] = cur->children[i]->name;
    }
    return match_count;
}

/* helpers for main_do_tab_complete: base_len computation */
static int tab_base_len(const char *buf) {
    char *last_space = strrchr(buf, ' ');
    return last_space ? (int)(last_space - buf) + 1 : 0;
}

/* helpers for main_do_tab_complete: single match apply */
static int tab_apply_single(char *line, int *pos, const char *prompt,
                            const char *buf, const char *completion) {
    int base_len = tab_base_len(buf);
    const char *suffix = completion + (*pos - base_len);
    int suffix_len = (int)strlen(suffix);
    if (*pos + suffix_len + 1 < MAX_LINE_LEN) {
        memcpy(line + *pos, suffix, suffix_len);
        line[*pos + suffix_len] = ' ';
        line[*pos + suffix_len + 1] = '\0';
        *pos = *pos + suffix_len + 1;
    }
    printf("\r\033[K%s%s", prompt, line);
    fflush(stdout);
    return 1;
}

/* helpers for main_do_tab_complete: multi match common-prefix apply */
static int tab_apply_multi(char *line, int *pos, const char *prompt,
                           const char *buf, const char **matches, int match_count) {
    int base_len = tab_base_len(buf);
    int cplen = main_common_prefix_len(matches, match_count);
    const char *first = matches[0];
    const char *suffix = first + (*pos - base_len);
    int new_cplen = cplen - (*pos - base_len);
    if (new_cplen < 0) new_cplen = 0;
    if (new_cplen > 0 && *pos + new_cplen < MAX_LINE_LEN) {
        memcpy(line + *pos, suffix, new_cplen);
        line[*pos + new_cplen] = '\0';
        *pos = *pos + new_cplen;
    }
    printf("\r\033[K%s%s\n", prompt, line);
    for (int i = 0; i < match_count; i++)
        printf("%s  ", matches[i]);
    printf("\n%s%s", prompt, line);
    fflush(stdout);
    return 1;
}

/* helpers for main_do_tab_complete: context-sensitive match collection
 * 根据 cmd 名字决定补全来源（子域 / 类型枚举 / 模式值）。
 * 把这组 if-else 链从主函数中抽出来，降低主函数圈复杂度。 */
static int tab_collect_for_cmd(const char *cmd, const char *partial,
                               int tc, int is_last_space,
                               const char **matches, int max_matches) {
    if (utils_str_eq(cmd, "cd") || utils_str_eq(cmd, "rm") || utils_str_eq(cmd, "ls"))
        return tab_collect_child_domains(partial, matches, max_matches);
    if (utils_str_eq(cmd, "find") && tc == 2 && !is_last_space)
        return main_try_complete(partial, g_domain_types, max_matches, matches);
    if (utils_str_eq(cmd, "mode"))
        return main_try_complete(partial, g_mode_values, max_matches, matches);
    if (utils_str_eq(cmd, "cmode"))
        return main_try_complete(partial, g_cmode_values, max_matches, matches);
    return tab_collect_child_domains(partial, matches, max_matches);
}

/*
 * main_do_tab_complete - perform tab completion on the current line.
 * Modifies line and updates pos. Returns 1 if display was refreshed.
 */
static int main_do_tab_complete(char *line, int *pos, const char *prompt)
{
    /* Make a working copy of the line up to cursor */
    char buf[MAX_LINE_LEN];
    strncpy(buf, line, *pos);
    buf[*pos] = '\0';

    /* Tokenize what's been typed so far */
    int tc = 0;
    char **tokens = tokenize(buf, &tc);

    const char *matches[128];
    int match_count = 0;

    if (tc == 0 || (tc == 1 && buf[*pos - 1] != ' ')) {
        /* Completing first token (command name) */
        const char *prefix = (tc > 0) ? tokens[0] : "";
        match_count = main_try_complete(prefix, g_cmd_names, 128, matches);
    } else {
        /* Completing second+ token - context-sensitive */
        const char *cmd = tokens[0];
        const char *partial = (tc > 1) ? tokens[tc - 1] : "";
        int is_last_space = (buf[*pos - 1] == ' ');
        match_count = tab_collect_for_cmd(cmd, partial, tc, is_last_space, matches, 128);
    }

    utils_free_tokens(tokens, tc);

    if (match_count == 0) {
        printf("\a");
        fflush(stdout);
        return 0;
    }
    if (match_count == 1)
        return tab_apply_single(line, pos, prompt, buf, matches[0]);
    return tab_apply_multi(line, pos, prompt, buf, matches, match_count);
}

/* ------------------------------------------------------------------ */
/* REPL history helpers                                               */
/* ------------------------------------------------------------------ */

#define CBOOT_HISTORY_MAX 100

static void main_repl_history_up(char *line, int *pos, const char *prompt,
                                  char **history, int hist_count,
                                  int *hist_pos, int *hist_browsing, char *saved_line)
{
    if (hist_count <= 0) return;
    if (!*hist_browsing) {
        strcpy(saved_line, line);
        *hist_browsing = 1;
        *hist_pos = hist_count;
    }
    if (*hist_pos > 0) {
        (*hist_pos)--;
        strcpy(line, history[*hist_pos]);
        printf("\r\033[K%s%s", prompt, line);
        fflush(stdout);
        *pos = (int)strlen(line);
    }
}

static void main_repl_history_down(char *line, int *pos, const char *prompt,
                                    char **history, int hist_count,
                                    int *hist_pos, int *hist_browsing, char *saved_line)
{
    if (!*hist_browsing) return;
    if (*hist_pos < hist_count - 1) {
        (*hist_pos)++;
        strcpy(line, history[*hist_pos]);
    } else {
        *hist_pos = hist_count;
        strcpy(line, saved_line);
        *hist_browsing = 0;
    }
    printf("\r\033[K%s%s", prompt, line);
    fflush(stdout);
    *pos = (int)strlen(line);
}

/* ------------------------------------------------------------------ */
/* REPL escape sequence helpers                                       */
/* ------------------------------------------------------------------ */

static void main_repl_consume_extended_esc(void) {
    char c;
    while (1) {
        if (read(STDIN_FILENO, &c, 1) != 1) break;
        if (c == '~' || (c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z')) break;
    }
}

static void main_repl_consume_bracketed_paste(void) {
    char c;
    while (1) {
        if (read(STDIN_FILENO, &c, 1) != 1) break;
        if (c == '~' || c == 'h' || c == 'l') break;
    }
}

/* ------------------------------------------------------------------ */
/* REPL input line reader                                             */
/* ------------------------------------------------------------------ */

/* 处理转义序列；返回 1 表示已处理，0 表示该字符不属于转义序列 */
/* ESC[ 后的字符处理：方向键/扩展序列 */
static void main_repl_handle_esc2(char c, char *line, int *pos, const char *prompt,
                                  char **history, int hist_count, int *hist_pos,
                                  int *hist_browsing, char *saved_line, int *esc_state) {
    if (c == 'A') {
        main_repl_history_up(line, pos, prompt, history, hist_count,
                             hist_pos, hist_browsing, saved_line);
    } else if (c == 'B') {
        main_repl_history_down(line, pos, prompt, history, hist_count,
                               hist_pos, hist_browsing, saved_line);
    } else if (c == '?') {
        *esc_state = 3;
        return;  /* 保持 state 3，不重置 */
    } else if (c >= '0' && c <= '9') {
        main_repl_consume_extended_esc();
    }
    *esc_state = 0;
}

static int main_repl_handle_esc(int *esc_state, char c, char *line, int *pos,
                                const char *prompt, char **history, int hist_count,
                                int *hist_pos, int *hist_browsing, char *saved_line) {
    if (*esc_state == 0 && c == '\033') { *esc_state = 1; return 1; }
    if (*esc_state == 1) {
        *esc_state = (c == '[') ? 2 : 0;
        return 1;
    }
    if (*esc_state == 2) {
        main_repl_handle_esc2(c, line, pos, prompt, history, hist_count,
                              hist_pos, hist_browsing, saved_line, esc_state);
        return 1;
    }
    if (*esc_state == 3) {
        if (c >= '0' && c <= '9') main_repl_consume_bracketed_paste();
        *esc_state = 0;
        return 1;
    }
    return 0;
}

/* 处理控制字符；返回 1 表示已处理（继续循环），2 表示换行（结束读取），0 表示非控制字符 */
static int main_repl_handle_ctrl(char c, char *line, int *pos, const char *prompt) {
    if (c == '\t') {
        if (*pos > 0) main_do_tab_complete(line, pos, prompt);
        return 1;
    }
    if (c == '\n' || c == '\r') {
        line[*pos] = '\0';
        printf("\n");
        return 2;
    }
    if (c == 3) {  /* Ctrl+C */
        line[0] = '\0'; *pos = 0;
        printf("^C\n%s", prompt);
        fflush(stdout);
        return 1;
    }
    if (c == 4) {  /* Ctrl+D */
        if (*pos == 0) { printf("\n"); g_running = 0; }
        return 1;
    }
    if (c == 12) {  /* Ctrl+L */
        printf("\033[2J\033[H%s%s", prompt, line);
        fflush(stdout);
        return 1;
    }
    if (c == 127 || c == '\b') {
        if (*pos > 0) {
            (*pos)--;
            line[*pos] = '\0';
            printf("\b \b");
            fflush(stdout);
        }
        return 1;
    }
    return 0;
}

static int main_repl_read_line(char *line, const char *prompt,
                                char **history, int hist_count,
                                int *hist_pos, int *hist_browsing, char *saved_line)
{
    int pos = 0;
    int esc_state = 0;
    line[0] = '\0';

    while (1) {
        char c;
        if (read(STDIN_FILENO, &c, 1) != 1) {
            g_running = 0;
            return 0;
        }

        /* ESC sequence state machine */
        if (main_repl_handle_esc(&esc_state, c, line, &pos, prompt,
                                 history, hist_count, hist_pos, hist_browsing, saved_line))
            continue;

        /* Control characters */
        int rc = main_repl_handle_ctrl(c, line, &pos, prompt);
        if (rc == 2) return 1;
        if (rc == 1) {
            if (!g_running) return 0;
            continue;
        }

        /* Regular characters */
        if (pos < MAX_LINE_LEN - 1 && c >= 32) {
            line[pos++] = c;
            line[pos] = '\0';
            putchar(c);
            fflush(stdout);
        }
    }
}

/* ------------------------------------------------------------------ */
/* REPL loop                                                          */
/* ------------------------------------------------------------------ */

static void main_repl_loop(void) {
    char line[MAX_LINE_LEN];
    char saved_line[MAX_LINE_LEN];
    char prompt[MAX_PATH_LEN];
    char *history[CBOOT_HISTORY_MAX] = {0};
    int hist_count = 0;
    int hist_pos = 0;
    int hist_browsing = 0;

    struct termios old_term, new_term;
    tcgetattr(STDIN_FILENO, &old_term);
    new_term = old_term;
    new_term.c_lflag &= ~(ICANON | ECHO);
    tcsetattr(STDIN_FILENO, TCSANOW, &new_term);

    printf("\033[?2004h");
    fflush(stdout);

    while (g_running) {
        char *path = domain_domain_get_path(g_proj->current);
        snprintf(prompt, sizeof(prompt), "CBoot%s> ", path);
        free(path);

        printf("%s", prompt);
        fflush(stdout);

        if (!main_repl_read_line(line, prompt, history, hist_count,
                                 &hist_pos, &hist_browsing, saved_line))
            break;

        hist_browsing = 0;

        char *trimmed = utils_trim(line);
        if (trimmed[0] == '\0') continue;

        /* Add to history */
        if (hist_count == 0 || strcmp(line, history[hist_count - 1]) != 0) {
            if (hist_count < CBOOT_HISTORY_MAX) {
                history[hist_count] = strdup(line);
                hist_count++;
            } else {
                free(history[0]);
                for (int i = 0; i < CBOOT_HISTORY_MAX - 1; i++)
                    history[i] = history[i + 1];
                history[CBOOT_HISTORY_MAX - 1] = strdup(line);
            }
        }
        hist_pos = hist_count;

        /* Tokenize and dispatch */
        int token_count = 0;
        char **tokens = tokenize(trimmed, &token_count);
        if (!tokens || token_count == 0) {
            utils_free_tokens(tokens, token_count);
            continue;
        }
        main_dispatch_command(tokens, token_count);
        utils_free_tokens(tokens, token_count);
    }

    printf("\033[?2004l");
    fflush(stdout);
    tcsetattr(STDIN_FILENO, TCSANOW, &old_term);

    for (int i = 0; i < hist_count; i++)
        free(history[i]);
}

/* ------------------------------------------------------------------ */
/* Helpers: join tokens[1..] into a single string                     */
/* ------------------------------------------------------------------ */

static void main_join_rest(char *buf, size_t size, char **tokens, int count) {
    buf[0] = '\0';
    for (int i = 1; i < count; i++) {
        if (i > 1) strncat(buf, " ", size - strlen(buf) - 1);
        strncat(buf, tokens[i], size - strlen(buf) - 1);
    }
}

/* ------------------------------------------------------------------ */
/* Command dispatchers by category                                    */
/* ------------------------------------------------------------------ */

/* 单参数建立域命令的分发表 */
static int main_dispatch_build(const char *cmd, char **tokens, int count) {
    /* 单参数命令：name + 函数指针 */
    struct { const char *name; int (*fn)(const char *); const char *usage; } single[] = {
        {"mod",    commands_cmd_mod,    "mod <名称>"},
        {"struct", commands_cmd_struct, "struct <名称>"},
        {"type",   commands_cmd_type,   "type <名称>"},
        {"def",    commands_cmd_def,    "def <名称>"},
    };
    for (size_t i = 0; i < sizeof(single)/sizeof(single[0]); i++) {
        if (!utils_str_eq(cmd, single[i].name)) continue;
        if (count < 2) { printf("用法: %s\n", single[i].usage); return -1; }
        return single[i].fn(tokens[1]);
    }

    /* 双参数命令：name + 函数指针 */
    struct { const char *name; int (*fn)(const char *, const char *); const char *usage; } typed[] = {
        {"void", commands_cmd_void, "void <名称> <返回类型>"},
        {"var",  commands_cmd_var,  "var <名称> <类型>"},
        {"mem",  commands_cmd_mem,  "mem <名称> <类型>"},
        {"enum", commands_cmd_enum, "enum <def1>,<def2>,... <start_num>"},
    };
    for (size_t i = 0; i < sizeof(typed)/sizeof(typed[0]); i++) {
        if (!utils_str_eq(cmd, typed[i].name)) continue;
        if (count < 3) { printf("用法: %s\n", typed[i].usage); return -1; }
        return typed[i].fn(tokens[1], tokens[2]);
    }
    return -2;
}

/* 读取多行代码直到 EOF 行，存入 code_buf */
static void main_read_code_from_stdin(char *code_buf, size_t size) {
    code_buf[0] = '\0';
    char line[MAX_LINE_LEN];
    int first = 1;
    while (fgets(line, sizeof(line), stdin)) {
        size_t len = strlen(line);
        if (len > 0 && line[len - 1] == '\n') line[len - 1] = '\0';
        if (utils_str_eq(line, "EOF")) break;
        if (!first) strcat(code_buf, "\n");
        strncat(code_buf, line, size - strlen(code_buf) - 1);
        first = 0;
    }
}

static int main_dispatch_modify(const char *cmd, char **tokens, int count) {
    /* 单参数命令 */
    if (utils_str_eq(cmd, "value")) {
        if (count < 2) { printf("用法: value <值>\n"); return -1; }
        return commands_cmd_value(tokens[1]);
    }
    if (utils_str_eq(cmd, "cmode")) {
        if (count < 2) { printf("用法: cmode <exe|sl|dl|normal>\n"); return -1; }
        return commands_cmd_cmode(tokens[1]);
    }

    /* 拼接式命令 (tokens[1..]) */
    struct { const char *name; int (*fn)(const char *); const char *usage; int strip; } joined[] = {
        {"cmt",  commands_cmd_cmt,  "cmt \"文本\"",       1},
        {"call", commands_cmd_call, "call <调用约定>",    0},
        {"mode", commands_cmd_mode, "mode <模式>",        0},
    };
    for (size_t i = 0; i < sizeof(joined)/sizeof(joined[0]); i++) {
        if (!utils_str_eq(cmd, joined[i].name)) continue;
        if (count < 2) { printf("用法: %s\n", joined[i].usage); return -1; }
        char buf[MAX_LINE_LEN] = {0};
        main_join_rest(buf, sizeof(buf), tokens, count);
        if (joined[i].strip) utils_strip_quotes(buf);
        return joined[i].fn(buf);
    }

    /* code 命令：无参数时读取多行，有参数时使用拼接 */
    if (utils_str_eq(cmd, "code")) {
        if (count < 2) {
            printf("输入代码，以 Ctrl+D 或 EOF 结束:\n");
            char code_buf[MAX_LINE_LEN * 64];
            main_read_code_from_stdin(code_buf, sizeof(code_buf));
            domain_domain_set_code(g_proj->current, code_buf);
            printf("code 已设置。\n");
            return 0;
        }
        char buf[MAX_LINE_LEN] = {0};
        main_join_rest(buf, sizeof(buf), tokens, count);
        domain_domain_set_code(g_proj->current, buf);
        printf("code 已设置。\n");
        return 0;
    }
    return -2;
}

/* 解析 rm 命令的 -f 标志和名称 */
static int main_parse_rm(char **tokens, int count, const char **name, int *force) {
    *name = NULL; *force = 0;
    for (int i = 1; i < count; i++) {
        if (utils_str_eq(tokens[i], "-f")) *force = 1;
        else *name = tokens[i];
    }
    if (!*name) { printf("用法: rm <名称> [-f]\n"); return -1; }
    return 0;
}

/* 解析 find 命令的 -a/-an 标志 */
static int main_parse_find_flags(char **tokens, int count) {
    int flags = 0;
    for (int i = 3; i < count; i++) {
        if (utils_str_eq(tokens[i], "-a")) flags = 1;
        else if (utils_str_startswith(tokens[i], "-a")) flags = atoi(tokens[i] + 2);
    }
    return flags;
}

static int main_dispatch_control(const char *cmd, char **tokens, int count) {
    if (utils_str_eq(cmd, "cd"))  return commands_cmd_cd(count >= 2 ? tokens[1] : NULL);
    if (utils_str_eq(cmd, "ls"))  return commands_cmd_ls(count >= 2 ? tokens[1] : NULL);
    if (utils_str_eq(cmd, "exit")) return commands_cmd_exit();
    if (utils_str_eq(cmd, "mv")) {
        if (count < 3) { printf("用法: mv <src> <target>\n"); return -1; }
        return commands_cmd_mv(tokens[1], tokens[2]);
    }
    if (utils_str_eq(cmd, "rm")) {
        const char *name; int force;
        if (main_parse_rm(tokens, count, &name, &force) != 0) return -1;
        return commands_cmd_rm(name, force);
    }
    if (utils_str_eq(cmd, "find")) {
        if (count < 3) { printf("用法: find <类型> <字符> [-a|-an]\n"); return -1; }
        return commands_cmd_find(tokens[1], tokens[2], main_parse_find_flags(tokens, count));
    }
    return -2;
}

static int main_dispatch_action(const char *cmd, char **tokens, int count) {
    (void)tokens;
    if (utils_str_eq(cmd, "gen")) {
        if (g_skip_gen) return 0;
        return commands_cmd_gen();
    }
    if (utils_str_eq(cmd, "analyze")) {
        return commands_cmd_analyze();
    }
    if (utils_str_eq(cmd, "update")) {
        return commands_cmd_update();
    }
    if (utils_str_eq(cmd, "adjust")) {
        return commands_cmd_adjust();
    }
    if (utils_str_eq(cmd, "im")) {
        if (count < 2) { printf("用法: im <.cboot 文件>\n"); return -1; }
        return commands_cmd_im(tokens[1]);
    }
    if (utils_str_eq(cmd, "in")) {
        if (count < 2) { printf("用法: in <.cboot 文件>\n"); return -1; }
        return commands_cmd_in(tokens[1]);
    }
    if (utils_str_eq(cmd, "res")) {
        if (count < 2) { printf("用法: res <资源文件>\n"); return -1; }
        return commands_cmd_res(tokens[1]);
    }
    return -2;
}

/* ------------------------------------------------------------------ */
/* Command dispatcher (新规范 v0.5.0)                                     */
/* ------------------------------------------------------------------ */

static int main_dispatch_command(char **tokens, int count) {
    const char *cmd = tokens[0];
    int rc;

    if (utils_str_eq(cmd, "help") || utils_str_eq(cmd, "?")) {
        main_print_usage("cboot");
        return 0;
    }

    rc = main_dispatch_build(cmd, tokens, count);
    if (rc != -2) return rc;

    rc = main_dispatch_modify(cmd, tokens, count);
    if (rc != -2) return rc;

    rc = main_dispatch_control(cmd, tokens, count);
    if (rc != -2) return rc;

    rc = main_dispatch_action(cmd, tokens, count);
    if (rc != -2) return rc;

    if (parser_try_cboot_ref(cmd)) {
        return 0;
    }

    printf("未知命令: %s (输入 help 查看帮助)\n", cmd);
    return -1;
}
